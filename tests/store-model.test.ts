...
function sortByPosition(left: TaskObj, right: TaskObj) {
  const lt = left.lastActivatedAt ? new Date(left.lastActivatedAt).getTime() : 0;
  const rt = right.lastActivatedAt ? new Date(right.lastActivatedAt).getTime() : 0;
...
test("sortByPosition: newer lastActivatedAt sorts first", () => {
  const older = task({ id: "older", position: 0, lastActivatedAt: "2026-01-01T10:00:00Z" });
  const newer = task({ id: "newer", position: 1, lastActivatedAt: "2026-01-02T10:00:00Z" });
...
test("sortByPosition: activated task sorts before never-activated", () => {
  const activated = task({ id: "activated", position: 1, lastActivatedAt: "2026-01-01T10:00:00Z" });
...
test("sortByPosition: section order preserved for never-activated tasks", () => {
  ...
  assert.deepEqual(sorted.map((t) => t.id), ["sec1", "sec2", "sec3"]);
});
